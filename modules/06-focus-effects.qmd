...
- __Highlighting__: the `highlight` effect is used to highlight parts of text or code of a sticky.
...
To shrink or enlarge a sticky element we use the `scale-by` effect. This 
attribute takes non-negative values. The figure below depicts the
...
To move a sticky element we use the `pan-to` effect. This attribute takes a 
pair of `x` and `y` values that control the horizontal and vertical panning,
respectively. The figure below depicts the resulting effect of `pan-to`---using 
different units---when applied on an image (but you cal also apply it on text):
...
To highlight certain lines of text in a sticky element we use the `highlight` 
effect. The way `highlight` works is by emphasizing certain parts of text
...
Often, though, we may be interested in highlighting just a single word, or some
part of a line of text. To obtain this type of highlighting we have to use 
