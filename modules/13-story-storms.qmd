...
different from the other stories because the images is in this case are all 
...
and finishing in 2022. The following figure shows the 11 graphics that I ended
up with.
...
The use of color is important: they different color hues match the progression 
...
progress to new graphics exposing certain pieces of information, one at a time.
...
up with a minimal, mostly empty, graphic.


### Hiding Graphical Elements

The deconstruction process that I follow involves hiding or making invisible
some of the displayed data, which is usually a combination of some of the 
bars, some of the axis tick marks, sometimes also some of the labels, etc.

Here's an example of what I mean. In the following snippet I show the commands
to get two versions of the same bar-chart. The difference is that the first 
set of commands gives you the bar chart in which all its elements are visible.
In contrast, the second set of commands give you a bar chart in which most of 
its graphical elements are hidden by making them transparent. 
...
You can take a look at the story of the Storms per Year in the North Atlantic
